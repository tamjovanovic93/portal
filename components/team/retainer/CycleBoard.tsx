...
function TaskCard({ task, projectId, isActive, roster = [] }: { task: Task; projectId: string; isActive: boolean; roster?: RosterMember[] }) {
  ...
  return (
    <div
      className={`rounded-md border bg-surface px-3 py-2.5 space-y-1.5 ${isPending ? "opacity-50" : ""} ${
        activeBlocker
    ...
    >
      {activeBlocker && (
        <span className="inline-flex items-center gap-1 text-xs px-1.5 py-0.5 rounded bg-rose-fill text-rose font-semibold">
          ⛔ Blocker
        </span>
      )}
      <div className="flex items-start justify-between gap-2">
        <button
          ...
          className={`text-sm text-left leading-snug flex-1 ${
            task.status === "DONE" ? "line-through text-ink-3" : "text-ink"
          }`}
        >
          {task.name}
        </button>
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className="text-xs text-ink-2">
          ...
          {task.ownerRole ? <span className="text-ink-4"> · {OWNER_ROLE_LABEL[task.ownerRole]}</span> : null}
        </span>
        {openClient > 0 && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-amber-fill text-amber font-medium">{openClient} client Q</span>
        )}
        {openConfirm > 0 && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-purple-100 text-purple font-medium">{openConfirm} to confirm</span>
        )}
        {openTeam > 0 && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-blue-fill text-blue font-medium">{openTeam} team Q</span>
        )}
        {isUnblocked && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-mint-fill text-mint font-medium">
            Unblocked {new Date(task.unblockedAt!).toLocaleDateString()}
          </span>
        )}
        {awaitingClient && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-amber-fill text-amber font-medium">Awaiting client</span>
        )}
        {clientApproved && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-mint-fill text-mint font-medium">Client approved</span>
        )}
      </div>

      {expanded && (
        <div className="space-y-2 pt-1">
          {task.description && <p className="text-xs text-ink-3">{task.description}</p>}
          {task.dueDate && (
            <p className={`text-xs ${isOverdue ? "text-rose font-medium" : "text-ink-2"}`}>
              ...
              {task.isBlocker && (
                isUnblocked ? (
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <button type="button" onClick={undoUnblock} disabled={isPending} className="text-ink-2 hover:text-ink underline underline-offset-2">
                      undo
                    </button>
                  </div>
                ) : (
          ...
          {roster.length > 0 && (
            <div className="pt-2 border-t border-line">
              <label className="block text-xs text-ink-2 mb-1">Questions</label>
      ...
      <div className="flex items-center justify-between gap-2 pt-0.5">
        {isActive ? (
          <select
            ...
            className="text-xs text-ink-2 rounded border border-line bg-surface px-1.5 py-1 focus:outline-none focus:ring-1 focus:ring-neutral-900 cursor-pointer"
          >
            {order.map((s) => (
          ...
          <span className="text-xs text-ink-2">{order[idx]?.label ?? task.status}</span>
        )}
        {isActive && (
...
function CommunicationSection({
  ...
  return (
    <div className="pt-2 border-t border-line space-y-2">
      <p className="text-xs font-medium text-ink-2">Communication / Questions <span className="text-ink-4 font-normal">(optional)</span></p>

      {pending.length > 0 && (
        <div className="space-y-1">
          {pending.map((q, i) => (
            <div key={i} className="flex items-start gap-2 text-xs bg-page border border-line rounded px-2 py-1">
              <span className={`shrink-0 px-1.5 py-0.5 rounded font-medium ${q.to === "client" ? "bg-amber-fill text-amber" : "bg-blue-fill text-blue"}`}>
                {q.to === "client" ? "Client" : q.recipientName}
              </span>
              <span className="flex-1 text-ink-2">{q.text}</span>
              <button type="button" onClick={() => onRemove(i)} className="shrink-0 text-ink-4 hover:text-red-500">×</button>
            </div>
          ))}
        </div>
      )}
  ...
  if (!editing) {
    return (
      <button
        ...
        className="text-xs text-ink-4 hover:text-white text-left transition-colors mt-1"
      >
  ...
  return (
    <div className="mt-2 flex items-start gap-2">
      ...
      <button
        ...
        className="text-xs px-2.5 py-1 rounded-md bg-surface text-ink hover:bg-surface-2 transition-colors"
      >
        Save
      </button>
    </div>
...
function CycleEditForm({ cycle, onDone }: { cycle: Cycle; onDone: () => void }) {
  ...
  return (
    <div className="flex items-end gap-2 flex-wrap">
      <div>
        <label className="block text-xs text-ink-4 mb-0.5">Name</label>
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          className="text-sm text-ink rounded border border-neutral-600 bg-neutral-800 px-2 py-1 focus:outline-none focus:ring-1 focus:ring-neutral-400 placeholder:text-neutral-500"
        />
      </div>
      <div>
        <label className="block text-xs text-ink-4 mb-0.5">Start</label>
        <input
          type="date"
          value={startDate}
      ...
      <div>
        <label className="block text-xs text-ink-4 mb-0.5">End</label>
        <input
          type="date"
          value={endDate}
      ...
      <button type="button" onClick={save} disabled={isPending} className="text-xs px-2.5 py-1.5 rounded-md bg-surface text-ink hover:bg-surface-2 transition-colors">
        {isPending ? "Saving…" : "Save"}
      </button>
      <button type="button" onClick={onDone} className="text-xs px-2.5 py-1.5 text-ink-4 hover:text-white transition-colors">Cancel</button>
    </div>
  );
}
...
function CloseCycleControl({ cycle, otherActiveCycles }: { cycle: Cycle; otherActiveCycles: { id: string; name: string }[] }) {
  ...
  if (!confirming) {
    return (
      <button type="button" onClick={() => setConfirming(true)} disabled={isPending} className="text-xs px-3 py-1.5 rounded-md border border-neutral-600 text-ink-4 hover:border-line-3 hover:text-white transition-colors">
        Close cycle
      </button>
    );
  }
  ...
  if (openCount === 0) {
    return (
      <div className="flex items-center gap-2">
        <span className="text-xs text-ink-4">Close this cycle?</span>
        <button type="button" onClick={() => close()} disabled={isPending} className="text-xs px-2.5 py-1 rounded-md bg-surface text-ink hover:bg-surface-2 transition-colors">Confirm</button>
        <button type="button" onClick={() => setConfirming(false)} className="text-xs text-ink-4 hover:text-white transition-colors">Cancel</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap justify-end max-w-md">
      <span className="text-xs text-ink-4">{openCount} open —</span>
      {otherActiveCycles.length > 0 && (
        <>
          ...
          <button type="button" onClick={() => close(target)} disabled={isPending || !target} className="text-xs px-2.5 py-1 rounded-md bg-surface text-ink hover:bg-surface-2 disabled:opacity-40 transition-colors">Move &amp; close</button>
        </>
      )}
      <button type="button" onClick={() => close()} disabled={isPending} className="text-xs px-2.5 py-1 rounded-md border border-neutral-600 text-ink-4 hover:text-white transition-colors">Close anyway</button>
      <button type="button" onClick={() => setConfirming(false)} className="text-xs text-ink-4 hover:text-white transition-colors">Cancel</button>
    </div>
  );
}
...
export default function CycleBoard({
  ...
  return (
    <div className={`border rounded-lg overflow-hidden ${isActive ? "border-neutral-900" : "border-line"} ${isPending ? "opacity-60" : ""}`}>
      {/* Cycle header */}
      <div className={`px-5 py-4 flex items-start justify-between gap-3 ${isActive ? "bg-neutral-900 text-white" : "bg-surface text-ink"}`}>
        <div className="min-w-0 flex-1">
          {editing && isActive ? (
            ...
            <>
              <div className="flex items-center gap-2">
                <h3 className={`text-sm font-semibold ${isActive ? "text-white" : "text-ink"}`}>{cycle.name}</h3>
                {!isActive && <span className="text-xs px-1.5 py-0.5 rounded bg-inset text-ink-3">Closed</span>}
              </div>
              ...
              {!tasks && (isActive ? <CycleFocus cycle={cycle} /> : cycle.focus && (
                <p className="text-xs text-ink-2 mt-1">Focus: {cycle.focus}</p>
              ))}
            </>
          )}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {isActive ? (
            <>
              {!tasks && (
                <button type="button" onClick={() => setEditing((v) => !v)} disabled={isPending} className="text-xs px-3 py-1.5 rounded-md border border-neutral-600 text-ink-4 hover:border-line-3 hover:text-white transition-colors">
                  {editing ? "Close" : "Edit"}
                </button>
              )}
              <button type="button" onClick={() => setShowAddTask((v) => !v)} disabled={isPending} className="text-xs px-3 py-1.5 rounded-md border border-neutral-600 text-ink-4 hover:border-line-3 hover:text-white transition-colors">+ Task</button>
              {tasks ? (
                <button type="button" onClick={handleDelete} disabled={isPending} className="text-xs px-3 py-1.5 rounded-md border border-neutral-600 text-ink-4 hover:border-line-3 hover:text-white transition-colors">Delete list</button>
              ) : (
        ...
        {cycle.tasks.length === 0 && !showAddTask ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {KANBAN_COLUMNS.map(({ key, label, statuses }) => {
              ...
              return (
                <div key={key} className="rounded-lg bg-surface border border-line p-2.5">
                  <div className="flex items-center justify-between mb-2 px-0.5">
                    ...
                    <span className={`text-xs font-medium px-1.5 rounded-full ${
                      col.length > 0 ? "bg-inset text-ink-2" : "text-ink-4"
                    }`}>
                      {col.length}
                    </span>
                  </div>
                  <div className={`space-y-2 min-h-[3rem] ${isDoneCol ? "opacity-80" : ""}`}>
                    {col.length === 0 ? (
                      <div className="h-12 rounded-md border border-dashed border-line" />
                    ) : (
