...
function TaskRow({
  ...
  return (
    <div
      className={`rounded-md border bg-surface px-3 py-2.5 space-y-2 ${isPending ? "opacity-60" : ""} ${
        activeBlocker ? "border-red-300 border-l-4 border-l-red-500" : "border-line"
      }`}
    >
      <div className="flex items-start justify-between gap-2">
        <span className={`text-sm ${task.status === "DONE" ? "line-through text-ink-4" : "text-ink"}`}>
          {task.name}
        </span>
        <div className="flex items-center gap-1.5 shrink-0">
          {activeBlocker && (
            <span className="text-xs px-1.5 py-0.5 rounded bg-rose-fill text-rose font-semibold">Blocked</span>
          )}
          <button
            type="button"
            onClick={() => run(() => deleteTask(task.id, projectId))}
            className="text-ink-4 hover:text-red-500 text-sm"
            title="Delete task"
          >
            ×
          </button>
        ...
        <button
          ...
          className={`text-xs px-2 py-1 rounded font-medium ${
            activeBlocker
              ...
              : "border border-red-200 text-rose hover:bg-red-50"
          }`}
        >
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {COLUMNS.map((col) => {
          ...
          return (
            <div key={col.key} className="rounded-lg bg-page border border-line p-2.5">
              <div className="flex items-center justify-between mb-2 px-0.5">
                <span className="text-xs font-semibold text-ink-2 uppercase tracking-wide">{col.label}</span>
                <span className={`text-xs font-medium px-1.5 rounded-full ${items.length > 0 ? "bg-neutral-200 text-ink-2" : "text-ink-4"}`}>
                  {items.length}
                </span>
              </div>
    ...
    <div className="space-y-5">
      {[...groups.entries()].map(([listName, list]) => (
        <div key={listName} className="space-y-2">
          <h4 className="text-sm font-semibold text-ink">{listName}</h4>
