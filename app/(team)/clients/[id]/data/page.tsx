...
// Shared CLIENT DATA — persistent information about the client/company
// (business, audience, messaging, strategy, brand). One canonical record per
// client, referenced by all of that client's projects. Project-specific
// information lives on each project (its Brief / workflow), not here.
...
export default async function ClientDataPage({
  ...
  const client = await prisma.profile.findUnique({
    where: { id: clientId },
    select: { name: true, email: true, role: true, projectsAsClient: { select: { id: true, name: true }, where: { isArchived: false }, orderBy: { createdAt: "asc" } } },
  });
  ...
  return (
    <div style={{ padding: "28px 32px 60px", maxWidth: 1100, margin: "0 auto" }} className="space-y-6">
      ...
      <nav className="faint flex items-center gap-1.5" style={{ fontSize: 12 }}>
        ...
        <span>›</span>
        <span style={{ color: "var(--text-2)" }}>Client Data</span>
      </nav>
      ...
      <div className="flex items-center justify-between gap-3 fade-up">
        <div>
          ...
          <p className="muted" style={{ fontSize: 13, marginTop: 6 }}>
            Shared across all of this client&apos;s projects
            {client.projectsAsClient.length > 0 && ` · ${client.projectsAsClient.length} project${client.projectsAsClient.length !== 1 ? "s" : ""}`}
          </p>
        </div>
        {profile && (
      ...
      {!profile && (
        <p className="muted card" style={{ fontSize: 13.5, padding: "16px 18px", borderStyle: "dashed" }}>
          No client profile yet. Run the intake pipeline from one of this client&apos;s projects to generate it.
        </p>
      )}
      ...
      >
        {TABS.map(({ id, label }) => (
          <Link
            key={id}
            href={`/clients/${clientId}/data?tab=${id}`}
      ...
      {tab === "business" && (
        <div className="space-y-8">
          <Snapshot
            title={str(company?.company_name) || clientName}
          ...
          <div className="space-y-6">
            ...
            <CompanyCard company={companyData} saveAction={upsertCompany.bind(null, clientId)} />
          ...
          <Snapshot
            ...
            summary={
              primaryObjective
                ...
                : "No strategy yet. Verify the profile and run Agent 2 from a project page."
            }
          />
      ...
      {tab === "brand" && (
        <div className="space-y-4">
          <Snapshot
            title="Brand Kit"
            summary="The central home for this client's brand identity — logo, typography, and colours. Shared across all of the client's projects; never duplicated."
          />
          <BrandKitCard
            clientId={clientId}
